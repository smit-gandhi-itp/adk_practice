...
async def update_interaction_history(
    ...
    """Append an interaction entry to session state using ADK events."""
    try:
        # Get current session
        ...
        # Existing history (safe copy)
        interaction_history = list(
            session.state.get("interaction_history", [])
        )
        ...
        event = Event(
            invocation_id=f"interaction_{int(time.time() * 1000)}",
            author="system",
        ...
        # ✅ THIS is the only correct way to persist state
...
async def process_agent_response(event):
    ...
    if event.author in ("system", None):
        return None

    if not hasattr(process_agent_response, "response_buffer"):
        process_agent_response.response_buffer = []
    ...
    if event.content and event.content.parts:
        for part in event.content.parts:
            if hasattr(part, "text") and part.text:
                ...
                # 🚫 Ignore tool calls (JSON-like payloads)
                if text.startswith("{") and '"name"' in text:
                    print("  ↪ Tool call detected, ignoring for user response")
                    return None

                process_agent_response.response_buffer.append(text)
                print(f"  Text: '{text}'")

    if event.is_final_response():
        if not process_agent_response.response_buffer:
            return None

        final_response = " ".join(process_agent_response.response_buffer)
        process_agent_response.response_buffer = []
    ...
    try:
        async for event in runner.run_async(
            ...
            if response:
                ...
                agent_name = event.author  # ✅ capture author ONLY for final response
